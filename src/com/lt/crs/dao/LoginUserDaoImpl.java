...
public class LoginUserDaoImpl implements LoginUserDaoInterface{
	...
	Connection con=null;
	CRSAdminMenu admin=null;
	CRSStudentMenu student=null;
	CRSProfessorMenu professor=null;
...
public void userLogin(String userId,String userPassword) throws Exception  {
	...
	if(a[0].trim().toLowerCase().equals("student".trim())) {
		...
		//	ps1=con.prepareStatement("select * from user_role where user_id=? and role_id=3");
			ps1=con.prepareStatement(SqlConstant.user_role_student);
			ps1.setInt(1, studID);
			ResultSet rs1=ps1.executeQuery();
			if(rs1.next()) {
				//ps2=con.prepareStatement("select * from student where student_id=? and student_password=?");
				ps2=con.prepareStatement(SqlConstant.student_login_query);
				ps2.setInt(1, studID);
		...
		try {			
			...
			ps3=con.prepareStatement(SqlConstant.user_role_professor);
			ps3.setInt(1, profID);
			...
			//	ps4=con.prepareStatement("select * from professor where professor_id=? and professor_password=?");
				ps4=con.prepareStatement(SqlConstant.professor_login_query);
				ps4.setInt(1, profID);
				...
				if(rs4.next()) {
					...
					professor=new CRSProfessorMenu();
		...
		try {
			...
			ps5=con.prepareStatement(SqlConstant.user_role_admin);
			ps5.setInt(1, adminID);
			ResultSet rs5=ps5.executeQuery();
			if(rs5.next()) {
			//	ps6=con.prepareStatement("select * from admin where admin_id=? and admin_password=?");
				ps6=con.prepareStatement(SqlConstant.admin_login_query);
...
public void updatePassword(String userId, String userPassword, String newpw) throws Exception {
	...
	if(a[0].trim().toLowerCase().equals("student".trim())) {
		...
		try {
			...
			if(rs1.next()) {
				...
				if(rs2.next()) {
					...
					ps7=con.prepareStatement(SqlConstant.student_password_update);
		...
		try {			
			...
			if(rs3.next()) {
				...
				if(rs4.next()) {
					...
					ps8=con.prepareStatement(SqlConstant.professor_password_update);
		...
		//			ps9=con.prepareStatement("update admin set admin_password=? where admin_id=?");
					ps9=con.prepareStatement(SqlConstant.admin_password_update);
